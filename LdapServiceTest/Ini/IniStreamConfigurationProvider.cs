            ...
            using (var reader = new StreamReader(stream))
            {
                ...
                while (reader.Peek() != -1)
                {
                    ...
                    if (separator < 0)
                    {
                        ...
                        // throw new FormatException(string.Format(LdapServiceTest.Ini.Resources.Error_UnrecognizedLineFormat, rawLine));

                        key = sectionPrefix + line.Trim();
                        value = ""; // This is WRONG ! Omitting a key on a NULL-value is WRONG ! 
                    }
                    else
                    {
                    ...
                    if (data.ContainsKey(key))
                    {
                        ...
                        throw new FormatException(string.Format(LdapServiceTest.Ini.Resources.Error_KeyIsDuplicated, key));
                    }
